...
import { useSelectedNode } from '@/packages/prototypes'
...
export interface IPathSelectorProps
    extends Omit<TreeSelectProps, 'onChange'> {
    value?: string
...
const transformDataSource = (node: TreeNode) => {
    ...
    const hasNoVoidChildren: (node: TreeNode) => boolean | undefined = (node: TreeNode) => {
    ...
    const findArrayParent: (node: TreeNode) => TreeNode | undefined = (node: TreeNode) => {
    ...
    const transformChildren: (node: TreeNode[], path?: string[]) => { label: string | undefined, value: any, node: any, children: any[] }[] = (children: TreeNode[], path = []) => {
